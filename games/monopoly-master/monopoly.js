function Game() {
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Auction <span id='propertyname'></span></div><div>Highest Bid = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, it is your turn to bid.</div<div><input id='bidprice' title='Enter an amount to bid on " + s.name + ".' style='width: 291px;' /></div><div><input id='bid' type='button' value='Bid' onclick='game.auctionBid();' title='Place your bid.' /><input id='bidpass' type='button' value='Pass' title='Skip bidding this time.' onclick='game.auctionPass();' /><input id='exitbid' type='button' value='Exit Auction' title='Stop bidding on " + s.name + " altogether.' onclick='if (confirm(\"Are you sure you want to stop bidding on this property altogether?\")) game.auctionExit();' /></div>", "blank");
